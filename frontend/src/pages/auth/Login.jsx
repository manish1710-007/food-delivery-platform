...
export default function Login() {
  ...
  const addLog = (msg) => {
    setAuthLogs(prev => [...prev, msg].slice(-5));
  };

  const handleInput = (setter, val, field) => {
    setter(val);
    if (Math.random() > 0.8) addLog(`> DETECTED_KEYSTROKE_IN_${field}`);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    setLoading(true);
    addLog("> VERIFYING_CIPHER...");

    try {
      const res = await api.post("/auth/login", { email, password });
      addLog("> ACCESS_GRANTED [OK]");
      
      // Artificial delay to show off the cool terminal logs
      setTimeout(() => {
      ...
      <div className="y2k-page container-fluid vh-100 p-0 overflow-hidden">
        ...
        <div className="row g-0 h-100 position-relative z-1">
          ...
          <div className="col-12 col-md-5 col-lg-4 d-flex flex-column justify-content-center align-items-center y2k-auth-panel border-end border-cyan position-relative">
            ...
            <div className="w-100 p-4 p-md-5 position-relative z-1" style={{ maxWidth: "450px" }}>
              
              {/* Header */}
              <div className="mb-5 text-start border-bottom-wire border-cyan pb-3">
                <div className="text-cyan font-monospace small mb-1">/// FOOD_DASH.exe</div>
                <h2 className="y2k-title text-main m-0 fs-3">
                  SYS_LOGIN <span className="blink text-cyan">_</span>
                </h2>
              </div>
              ...
              {/* Form */}
              <form onSubmit={handleSubmit} className="d-flex flex-column gap-4 text-start">
                
                {/* Email */}
                <div>
                  <label className="d-block text-muted small font-monospace mb-1">&gt; USR_ID (Email)</label>
                  <div className="y2k-input-group d-flex">
                    ...
                    <input
                      ...
                      value={email}
                      onChange={(e) => handleInput(setEmail, e.target.value, "USR_ID")}
                ...
                {/* Password */}
                <div>
                  <label className="d-block text-muted small font-monospace mb-1">&gt; CIPHER (Password)</label>
                  <div className="y2k-input-group d-flex">
                    ...
                    <input
                      ...
                      className="y2k-input flex-grow-1 p-2"
                      value={password}
                      onChange={(e) => handleInput(setPassword, e.target.value, "CIPHER")}
                      placeholder="••••••••"
                      required
                      disabled={loading}
                    />
                ...
                {/* Terminal Log Output */}
                <div className="y2k-wire-box border-cyan p-2 font-monospace text-muted mt-2" style={{ fontSize: "0.65rem", height: "80px", overflow: "hidden" }}>
                  {authLogs.map((log, i) => (
                ...
                <button
                  type="submit"
                  className="y2k-btn-magenta w-100 py-3 mt-2 d-flex justify-content-center align-items-center"
                  disabled={loading}
                >
                  {loading ? "[ PROCESSING_HANDSHAKE... ]" : "[ INITIATE_CONNECTION ]"}
                </button>
              </form>

              <div className="text-center font-monospace mt-5 small border-top-wire border-cyan pt-3">
                <span className="text-muted">NEW_NODE_DETECTED? </span>
                <Link to="/register" className="text-cyan text-decoration-none fw-bold hover-glow">
                  [ REGISTER_HERE ]
                </Link>
              </div>
          ...
          <div className="col-md-7 col-lg-8 d-none d-md-block position-relative y2k-hero-panel bg-dark overflow-hidden">
            ...
            <div className="position-absolute top-0 start-0 w-100 h-100 pointer-events-none p-4 d-flex flex-column justify-content-between">
              ...
              {/* Top Right HUD */}
              <div className="text-end font-monospace text-cyan small opacity-75">
                <div>SYS_TIME: {new Date().toISOString().split('T')[1].substring(0, 8)}</div>
                <div>LOCAL_HOST: ACTIVE</div>
              ...
              {/* Bottom Left Fake Boot Text */}
              <div className="font-monospace text-cyan small opacity-75" style={{ fontSize: "0.7rem", textShadow: "0 0 5px rgba(0, 229, 255, 0.5)" }}>
                <div> LOADING KERNEL... [OK]</div>
                <div> MOUNTING ROOT FILESYSTEM... [OK]</div>
                <div> ESTABLISHING SOCKET_CONNECTION... [OK]</div>
                <div> AWAITING_USER_AUTH</div>
                <h1 className="y2k-title text-white mt-3 fs-2 opacity-100" style={{ letterSpacing: "2px" }}>
                  FAST. FRESH.<br/>DELIVERED.
                </h1>
              </div>
